...
function adjustGamepadPosition() {
  ...
  if (isMobileDevice) {
    ...
    if (isLandscape) {
      ...
      // Espaçamento entre os botões em modo coluna
      const buttons = controls.querySelectorAll('button');
      buttons.forEach(button => {
        button.style.margin = '5px 0';
      });
      
      // Verificar se há espaço suficiente para afastar mais os controles
      if (window.innerWidth >= 768) {
        gamepadContainer.style.left = '20px';
        controls.style.right = '20px';
      }
      
      // Ajustar transparência dos controles
      ...
      // Restaurar as margens dos botões
      const buttons = controls.querySelectorAll('button');
      buttons.forEach(button => {
        button.style.margin = '';
      });
    ...
    // Desktop - botões abaixo do canvas
    gamepadContainer.style.position = '';
    gamepadContainer.style.left = '';
    gamepadContainer.style.bottom = '';
    ...
    if (scoreEl) {
      ...
      scoreEl.style.top = '';
      scoreEl.style.right = '';
      scoreEl.style.margin = '10px 0 0 0';
      scoreEl.style.fontSize = '';
      scoreEl.style.color = '';
      scoreEl.style.textShadow = '';
      scoreEl.style.backgroundColor = '';
...
function optimizeControlsVisibility() {
  ...
  if (isMobileDevice) {
    ...
    if (isRunning) {
      ...
      // Botões de jogo menos visíveis durante o jogo ativo, exceto o pauseBtn
